...
u64 sli_monitor_singal(struct cgroup *cgrp, u32 hevent, u32 levent, u32 count)
{
	return 0;
}
	...
	list_for_each_entry_rcu(event, &event_monitor->event_head, event_node) {
		...
		case SLI_SCHED_EVENT:
			if (!event_monitor->overrun) {
				...
				if (statistics >= event_monitor->schedlat_count[event->event_id])
					sli_monitor_singal(event_monitor->cgrp, event->event_type,
		...
		case SLI_MEM_EVENT:
			if (!event_monitor->overrun) {
				...
				if (statistics >= event_monitor->memlat_count[event->event_id])
					sli_monitor_singal(event_monitor->cgrp, event->event_type,
							   event->event_id, statistics);
			} else {
				event_monitor->overrun = 0;
		...
		case SLI_LONGTERM_EVENT:
			statistics = sli_get_longterm_statistics(event_monitor->cgrp,
								 event->event_id);
			if (!event_monitor->overrun) {
				expired_time = (u64)atomic_long_read(
					&event_monitor->longterm_statistics[event->event_id]) +
					event_monitor->longterm_threshold[event->event_id];

				/* Deal with time wrapping correclty */
				if ((long)(statistics - expired_time) >= 0)
					sli_monitor_singal(event_monitor->cgrp, event->event_type,
							   event->event_id,
							   (u32)(statistics - expired_time));
			} else {
				event_monitor->overrun = 0;
			}
